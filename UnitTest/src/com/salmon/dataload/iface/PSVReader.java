...
public class PSVReader
    ...
    * Constructor
    *
    * @param r         input Reader source of PSV Fields to read.
    *
    * @param separator
   ...
   /**
    * Reader source of the PSV fields to be read.
    */
   ...
   */
   ...
   /**
    * Read one field from the PSV file
    *
               ...
               case IN_PLAIN:
                  {  /* in middle of ordinary field */
                     switch ( category )
                        {
                        case QUOTE:
                           throw new IOException("Malformed PSV stream. Missing quote at start of field on line "+lineCount);
                     ...
                     switch ( category )
                        ...
                        case EOL:
                           if ( allowMultiLineFields )
                              ...
                              allFieldsDone = true;
                              throw new IOException ("Malformed PSV stream. Missing quote (\") after field on line "+lineCount);
                              }
                        case WHITESPACE:
                           field.append(' ');
                           break;
               ...
               case AFTER_END_QUOTE:
                  {
                     ...
                     switch ( category )
                        ...
                        case ORDINARY:
                           throw new IOException("Malformed PSV stream, missing separator after fieldon line "+lineCount);
                        }
                     break;
                  }
               ...
               case SKIPPING_TAIL:
                  {
                     ...
                     switch ( category )
                        ...
                        case QUOTE:
                        case ORDINARY:
                           throw new IOException("Malformed PSV stream, missing separator after field on line "+lineCount);
                        }
                     break;
                  }
   ...
   */
   ...
   /**
    * Read one integer field from the PSV file
    *
    ...
    */
   private int getInt() throws EOFException, IOException, NumberFormatException
   {
      String s = get();
      if ( s == null )
         {
   ...
   /**
   * Read one long field from the PSV file
   *
   ...
   * @exception NumberFormatException, if field does not contain a well-formed int.
   */
   private long getLong() throws EOFException, IOException, NumberFormatException
   {
      String s = get();
      if ( s == null )
         {
   ...
   /**
   * Read one float field from the PSV file.
   *
   ...
   * @exception NumberFormatException, if field does not contain a well-formed int.
   */
   private float getFloat() throws EOFException, IOException, NumberFormatException
   {
      String s = get();
      if ( s == null )
         {
   ...
   /**
   * Read one double field from the PSV file.
   *
   ...
   * @exception NumberFormatException, if field does not contain a well-formed int.
   */
//   private double getDouble() throws EOFException, IOException, NumberFormatException
//   {
//      String s = get();
//      if ( s == null )
//         {
...
//   }
   ...
   private void getLineIfNeeded() throws EOFException, IOException
   {
      ...
      if ( line == null )
         ...
         if ( r == null )
            {
            throw new IllegalArgumentException("attempt to use a closed PSVReader");
            }
         allFieldsDone = false;
   ...
   public static void main(String[] args)
      ...
      if ( DEBUGGING )
         ...
         try
            {
            // read test file
            PSVReader psv = new PSVReader(new FileReader("C:\\WCDE_ENT70\\workspace\\DataExchange\\src\\com\\mindprod\\psv\\validated.mas.import.psv"), '|', '\"', true, false);
            try
               {
               while ( true )
                  {
                  System.out.println("--> " + psv.get());
                  }
               }
